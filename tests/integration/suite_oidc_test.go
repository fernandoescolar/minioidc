...
type ErrorResponse struct {
	// Error is a single ASCII error code.
	Error string `json:"error"`

	// ErrorDescription is a human-readable ASCII text providing additional information, used to assist the client developer in understanding the error that occurred.
...
type TokenResponse struct {
	// AccessToken is a token that can be sent to a server to access resources.
	...
	// State is the state parameter from the client request.
	State *string `json:"state,omitempty"`

	// Scope is the scope parameter from the client request.
