...
func (gs *miniGrantStore) NewCodeGrant(id string, client domain.Client, session domain.Session, issuedAt, expiresAt time.Time, scopes []string, nonce string, codeChallenge, codeChallengeMethod string) (domain.Grant, error) {
...
func (gs *miniGrantStore) NewRefreshTokenGrant(id string, client domain.Client, session domain.Session, issuedAt, expiresAt time.Time, scopes []string) (domain.Grant, error) {
	scopesStr := strings.Join(scopes, " ")
	grant := &miniGrant{
		id:        id,
